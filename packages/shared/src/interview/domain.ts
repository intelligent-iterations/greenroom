...
/**
 * The interview-coaching domain.
 *
 * Everything here describes one *kind* of voice agent: a coach that pitches
 * language at a CEFR level and difficulty at a seniority bar, and remembers
 * what the learner is bad at. None of it belongs in the core, which is why it
 * sits behind `@greenroom/shared/interview` rather than the main entry — the
 * core has no opinion about whether an agent is teaching anyone anything.
 *
 * It stays in the repository because a platform with no worked example is a
 * pile of interfaces. This is the worked example.
 */
